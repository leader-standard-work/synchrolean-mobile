...
export class MetricsService {
  ...
  public getTeamCompletionRate(
    ...
    const endpoint = encodeURI(
      this.authService.url +
        `/api/tasks/metrics/team/${teamId}/${startDate}/${endDate}`
    );
  ...
  public getMemberCompletionRate(
    teamid: number,
    memberEmail: string,
    startDate: string,
    endDate: string
    ...
    const endpoint = encodeURI(
      this.authService.url +
        `/api/tasks/metrics/user/team/${teamid}/${startDate}/${endDate}/${memberEmail}`
    );
    return this.http.get<number>(endpoint);
  }
}
